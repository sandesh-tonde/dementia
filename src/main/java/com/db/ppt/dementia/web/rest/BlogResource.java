...
/**
 * REST controller for managing {@link com.db.ppt.dementia.domain.Blog}.
 */
@RestController
...
public class BlogResource {
    ...
    public ResponseEntity<Blog> createBlog(@Valid @RequestBody Blog blog) throws URISyntaxException {
        ...
        if (Objects.isNull(blog.getAuther())) {
            throw new BadRequestAlertException("Invalid association value provided", ENTITY_NAME, "null");
        }
        Long userId = blog.getAuther().getId();
        userRepository.findById(userId).ifPresent(blog::auther);
    ...
    public ResponseEntity<Blog> partialUpdateBlog(
        ...
        Optional<Blog> result = blogRepository
            ...
            .map(existingBlog -> {
                ...
                if (blog.getDateTime() != null) {
                    existingBlog.setDateTime(blog.getDateTime());
                }
                if (blog.getVote() != null) {
                    existingBlog.setVote(blog.getVote());
                }
    ...
    public ResponseEntity<List<Blog>> getAllBlogs(
        ...
        Page<Blog> page;
        if (eagerload) {
            page = blogRepository.findAllWithEagerRelationships(pageable);
        } else {
