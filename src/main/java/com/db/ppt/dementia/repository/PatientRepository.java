...
public interface PatientRepository extends JpaRepository<Patient, Long> {
    ...
    @Query(
        value = "select patient from Patient patient left join fetch patient.user_petient",
        countQuery = "select count(patient) from Patient patient"
    )
    Page<Patient> findAllWithToOneRelationships(Pageable pageable);

    @Query("select patient from Patient patient left join fetch patient.user_petient")
    List<Patient> findAllWithToOneRelationships();

    @Query("select patient from Patient patient left join fetch patient.user_petient where patient.id =:id")
