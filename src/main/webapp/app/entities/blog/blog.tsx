  ...
  return (
    <div>
      <h2 id="blog-heading" data-cy="BlogHeading">
        <Translate contentKey="dementiaApp.blog.home.title">Blogs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="dementiaApp.blog.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/blog/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="dementiaApp.blog.home.createLabel">Create new Blog</Translate>
          </Link>
        </div>
      </h2>
        ...
        {blogList && blogList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="dementiaApp.blog.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('vote')}>
                  <Translate contentKey="dementiaApp.blog.vote">Vote</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('vote')} />
                </th>
                <th>
                  <Translate contentKey="dementiaApp.blog.auther">Auther</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {blogList.map((blog, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{blog.content}</td>
                  <td>{blog.dateTime ? <TextFormat type="date" value={blog.dateTime} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{blog.vote}</td>
                  <td>{blog.auther ? blog.auther.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/blog/${blog.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="dementiaApp.blog.home.notFound">No Blogs found</Translate>
            </div>
          )
        )}
      </div>
