  ...
  return (
    <div>
      <h2 id="comment-heading" data-cy="CommentHeading">
        <Translate contentKey="dementiaApp.comment.home.title">Comments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="dementiaApp.comment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/comment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="dementiaApp.comment.home.createLabel">Create new Comment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {commentList && commentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="dementiaApp.comment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="dementiaApp.comment.addedBy">Added By</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="dementiaApp.comment.blog_comments">Blog Comments</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {commentList.map((comment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{comment.dateTime ? <TextFormat type="date" value={comment.dateTime} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{comment.addedBy ? comment.addedBy.login : ''}</td>
                  <td>{comment.blog_comments ? <Link to={`/blog/${comment.blog_comments.id}`}>{comment.blog_comments.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="dementiaApp.comment.home.notFound">No Comments found</Translate>
            </div>
          )
        )}
      </div>
